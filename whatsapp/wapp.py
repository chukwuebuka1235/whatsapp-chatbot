...
def process_message(data):
    try:
        ...
        if messages:
            ...
            if message.get('type') == 'text' and message['text']['body'].strip().lower() == 'cancel':
                # Clear all states for this user
                for state_dict in active_states:
                    if phone_number in state_dict:
                        del state_dict[phone_number]
                
                # Send welcome menu
                msg_payload = welcome_message(profile_name, "3298868791", 0.00)
                send_whatsapp_interactive(phone_number, msg_payload)
                return make_response("Process cancelled", 200)
            ...
            if phone_number in user_airtime_state:
                ...
                # === CANCELLATION CHECK FOR AIRTIME ===
                if message.get('type') == 'text' and message['text']['body'].strip().lower() == 'cancel':
                    del user_airtime_state[phone_number]
                    msg_payload = welcome_message(profile_name, "3298868791", 0.00)
                    send_whatsapp_interactive(phone_number, msg_payload)
                    return make_response("Airtime purchase cancelled", 200)
            ...
            if phone_number in user_transfer_state:
                ...
                # === CANCELLATION CHECK FOR TRANSFER ===
                if message.get('type') == 'text' and message['text']['body'].strip().lower() == 'cancel':
                    del user_transfer_state[phone_number]
                    msg_payload = welcome_message(profile_name, "3298868791", 0.00)
                    send_whatsapp_interactive(phone_number, msg_payload)
                    return make_response("Transfer cancelled", 200)
                # === END CANCELLATION CHECK ===
            ...
            if phone_number in user_data_state:
                ...
                # === CANCELLATION CHECK FOR DATA ===
                if message.get('type') == 'text' and message['text']['body'].strip().lower() == 'cancel':
                    del user_data_state[phone_number]
                    msg_payload = welcome_message(profile_name, "3298868791", 0.00)
                    send_whatsapp_interactive(phone_number, msg_payload)
                    return make_response("Data purchase cancelled", 200)
                # === END CANCELLATION CHECK ===
            ...
            if phone_number in user_electricity_state:
                ...
                # === CANCELLATION CHECK FOR ELECTRICITY ===
                if message.get('type') == 'text' and message['text']['body'].strip().lower() == 'cancel':
                    del user_electricity_state[phone_number]
                    msg_payload = welcome_message(profile_name, "3298868791", 0.00)
                    send_whatsapp_interactive(phone_number, msg_payload)
                    return make_response("Electricity payment cancelled", 200)
                # === END CANCELLATION CHECK ===
            ...
            if phone_number in user_cable_state:
                ...
                # === CANCELLATION CHECK FOR CABLE ===
                if message.get('type') == 'text' and message['text']['body'].strip().lower() == 'cancel':
                    del user_cable_state[phone_number]
                    msg_payload = welcome_message(profile_name, "3298868791", 0.00)
                    send_whatsapp_interactive(phone_number, msg_payload)
                    return make_response("Cable payment cancelled", 200)
                # === END CANCELLATION CHECK ===
            ...
            if phone_number in user_education_state:
                ...
                # === CANCELLATION CHECK FOR EDUCATION ===
                if message.get('type') == 'text' and message['text']['body'].strip().lower() == 'cancel':
                    del user_education_state[phone_number]
                    msg_payload = welcome_message(profile_name, "3298868791", 0.00)
                    send_whatsapp_interactive(phone_number, msg_payload)
                    return make_response("Education payment cancelled", 200)
                # === END CANCELLATION CHECK ===
            ...
            if phone_number in user_betting_state:
                ...
                # === CANCELLATION CHECK FOR BETTING ===
                if message.get('type') == 'text' and message['text']['body'].strip().lower() == 'cancel':
                    del user_betting_state[phone_number]
                    msg_payload = welcome_message(profile_name, "3298868791", 0.00)
                    send_whatsapp_interactive(phone_number, msg_payload)
                    return make_response("Betting payment cancelled", 200)
                # === END CANCELLATION CHECK ===
            ...
            if phone_number in user_bill_state:
                ...
                # === CANCELLATION CHECK FOR BILL SELECTION ===
                if message.get('type') == 'text' and message['text']['body'].strip().lower() == 'cancel':
                    del user_bill_state[phone_number]
                    msg_payload = welcome_message(profile_name, "3298868791", 0.00)
                    send_whatsapp_interactive(phone_number, msg_payload)
                    return make_response("Bill selection cancelled", 200)
                # === END CANCELLATION CHECK ===
            ...
            if message.get("type") == "interactive":
                ...
                if interactive.get("type") == "list_reply":
                    ...
                    # Check if user is already in any process
                    is_active = any(phone_number in state for state in active_states)
                    
                    if is_active and selected_id in ["transfer_money", "buy_airtime", "buy_data", "pay_bills"]:
                        send_whatsapp_message(
                            phone_number,
                            "⚠️ Please complete your current process first!\n"
                            "Type 'CANCEL' to end the current process"
                        )
                        return make_response("Process already active", 200)
                    # === END CONCURRENT PROCESS PREVENTION ===
